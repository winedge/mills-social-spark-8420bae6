...
import { formatReservationMessage, formatSpaceMessage, formatTableConfirmation, formatSpaceConfirmation, } from "./whatsapp";
 ...
 */
async function sendViaCallMeBot(to, body) {
    const key = process.env.CALLMEBOT_API_KEY;
    const num = digits(to);
    if (!key || !num)
        return false;
    try {
        const url = `https://api.callmebot.com/whatsapp.php?phone=%2B${num}&text=${encodeURIComponent(body)}&apikey=${encodeURIComponent(key)}`;
...
export async function sendWhatsApp(to, body, allowRelay = false) {
    if (await sendViaCloudApi(to, body))
        return true;
    if (allowRelay && (await sendViaCallMeBot(to, body)))
        return true;
    return false;
}
export async function notifyTable(r) {
...
export async function notifySpace(r) {
    const adminNumber = await getAdminNumber();
    const [adminSent, customerSent] = await Promise.all([
