...
import { useJobListings } from "@/lib/content";
import { supabase } from "@/integrations/supabase/client";
import { Briefcase, MapPin, Send, CheckCircle2, Loader2, Users, Heart, Star, Upload, FileText, X as CloseIcon } from "lucide-react";
...
function CareersPage() {
    ...
    return (<div className="bg-background text-foreground font-body min-h-screen">
      ...
      <section className="relative pt-8 pb-6 px-6 overflow-hidden border-b border-border">
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          <div className="text-center max-w-4xl mx-auto mb-10">
            ...
            <div className="mb-4">
              <WarpText text="WORK WITH US" color="#ffffff" warpStrength={0.08} warpScale={1.7} speed={0.55} pointerInfluence={0.42} pointerStrength={0.38} refraction={0.018} ripple fontSize="clamp(3.5rem, 12vw, 10rem)" fontWeight={800} style={{ height: '200px' }}/>
            </div>
      ...
      <section className="py-24 px-6 max-w-7xl mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
          <div>
            ...
            {jobsLoading ? (<div className="space-y-4">
                {[1, 2, 3].map((i) => (<div key={i} className="h-24 w-full bg-surface animate-pulse border border-border"/>))}
              </div>) : jobs.length === 0 ? (<div className="p-12 border-2 border-dashed border-border text-center rounded-lg">
                ...
                {jobs.map((job) => (<button key={job.id} onClick={() => {
                    setSelectedJob(job);
                    setSubmitted(false);
                }} className={`w-full text-left p-6 border transition-all ${selectedJob?.id === job.id
          ...
          <div className="relative">
            {selectedJob ? (<div className="sticky top-32 p-8 border-2 border-border bg-card shadow-[12px_12px_0px_0px_rgba(255,255,255,0.05)]">
                {submitted ? (<div className="text-center py-12">
                    ...
                    <button onClick={() => setSubmitted(false)} className="px-8 py-3 bg-accent text-primary-foreground font-bold uppercase tracking-widest text-sm hover:scale-105 transition-transform">
                      Back to Jobs
                    </button>
                  </div>) : (<>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="space-y-2">
                          ...
                          <input required name="fullName" className="w-full bg-background border border-border p-3 outline-none focus:border-accent transition-colors" placeholder="John Doe"/>
                        </div>
                        <div className="space-y-2">
                          <label className="font-mono text-[10px] uppercase tracking-[0.2em] text-muted-foreground">Email Address</label>
                          <input required type="email" name="email" className="w-full bg-background border border-border p-3 outline-none focus:border-accent transition-colors" placeholder="john@example.com"/>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="space-y-2">
                          ...
                          <input required name="phone" className="w-full bg-background border border-border p-3 outline-none focus:border-accent transition-colors" placeholder="(555) 000-0000"/>
                        </div>
                        <div className="space-y-2">
                          <label className="font-mono text-[10px] uppercase tracking-[0.2em] text-muted-foreground">Resume (PDF, Max 5MB)</label>
                          <div onClick={() => fileInputRef.current?.click()} className="w-full bg-background border border-border border-dashed p-3 min-h-[46px] flex items-center justify-between cursor-pointer hover:border-accent transition-colors">
                            ...
                            {resumeFile && (<button type="button" onClick={(e) => {
                        e.stopPropagation();
                        setResumeFile(null);
                      ...
                      <div className="space-y-2">
                        ...
                        <textarea name="message" rows={4} className="w-full bg-background border border-border p-3 outline-none focus:border-accent transition-colors resize-none" placeholder="Tell us why you'd be a great fit..."/>
                      </div>

                      <button type="submit" disabled={isSubmitting} className="w-full py-4 bg-accent text-primary-foreground font-black uppercase tracking-widest text-sm hover:brightness-110 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 transition-all">
                        {isSubmitting ? (<>
              ...
              </div>) : (<div className="h-full min-h-[400px] border-2 border-dashed border-border rounded-lg flex flex-col items-center justify-center p-12 text-center opacity-60">
      ...
      <section className="py-24 px-6 border-t border-border bg-surface/30">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="bg-card border-2 border-border p-8 md:p-12 shadow-[12px_12px_0px_0px_rgba(56,189,248,0.1)]">
            {submitted && !selectedJob ? (<div className="text-center py-12">
                <div className="size-20 rounded-full bg-green-500/10 text-green-500 flex items-center justify-center mx-auto mb-6">
                  <CheckCircle2 className="size-10"/>
                </div>
              ...
              </div>) : (<form onSubmit={(e) => handleSubmit(e)} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <label className="font-mono text-[10px] uppercase tracking-[0.2em] text-muted-foreground">Full Name</label>
                    <input required name="fullName" className="w-full bg-background border border-border p-3 outline-none focus:border-accent transition-colors" placeholder="Your Name"/>
                  </div>
                  <div className="space-y-2">
                    <label className="font-mono text-[10px] uppercase tracking-[0.2em] text-muted-foreground">Email Address</label>
                    <input required type="email" name="email" className="w-full bg-background border border-border p-3 outline-none focus:border-accent transition-colors" placeholder="your@email.com"/>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-2">
                    <label className="font-mono text-[10px] uppercase tracking-[0.2em] text-muted-foreground">Resume (PDF, Max 5MB)</label>
                    <div onClick={() => generalFileInputRef.current?.click()} className="w-full bg-background border border-border border-dashed p-3 min-h-[46px] flex items-center justify-between cursor-pointer hover:border-accent transition-colors">
                      <div className="flex items-center gap-2 overflow-hidden">
                        {resumeFile ? (<>
                            <FileText className="size-4 text-accent shrink-0"/>
                            <span className="text-xs truncate">{resumeFile.name}</span>
                    ...
                    <input ref={generalFileInputRef} type="file" accept=".pdf,.doc,.docx" onChange={handleFileChange} className="hidden"/>
                  </div>
                </div>

                <div className="space-y-2">
                  <label className="font-mono text-[10px] uppercase tracking-[0.2em] text-muted-foreground">Tell us about yourself</label>
                  <textarea name="message" rows={4} className="w-full bg-background border border-border p-3 outline-none focus:border-accent transition-colors resize-none" placeholder="Experience, interests, and what you're looking for..."/>
                </div>
