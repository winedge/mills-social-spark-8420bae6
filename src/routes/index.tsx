...
const DEFAULT_MARQUEE_IMAGES = [
  ...
  "/slider/venue-cabana-lounge.webp",
  ...
  "/slider/venue-circular-lounge.webp",
  "/slider/hh-fries.jpg",
...
function Home() {
  ...
  const dailySpecials = dbSpecials.length
    ? dbSpecials.map((s, i) => ({
        img: (s.image_url ? s.image_url : specialImgs[i % specialImgs.length]) as any,
        day: s.day,
        badge: s.badge,
        title: s.title,
        desc: s.description,
  ...
  const schedule = dbPulse.length
    ? dbPulse.map((s, i) => ({
        ...
        copy: s.copy,
        accent: s.accent,
        img: (s.image_url ? s.image_url : pulseImgs[i % pulseImgs.length]) as any,
      }))
    : fallbackSchedule;
