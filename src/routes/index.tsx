...
function Home() {
  ...
  const dailySpecials = dbSpecials.length
    ? dbSpecials.map((s, i) => ({
        img: (s.image_url ? (s.image_url.startsWith('/') ? s.image_url : s.image_url) : specialImgs[i % specialImgs.length]) as string,
        day: s.day,
        badge: s.badge,
        title: s.title,
        desc: s.description,
  ...
  const schedule = dbPulse.length
    ? dbPulse.map((s, i) => ({
        ...
        copy: s.copy,
        accent: s.accent,
        img: (s.image_url ? (s.image_url.startsWith('/') ? s.image_url : s.image_url) : pulseImgs[i % pulseImgs.length]) as string,
      }))
    : fallbackSchedule;
